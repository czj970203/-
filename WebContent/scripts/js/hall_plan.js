...
function init() {
	...
	$.ajax({
		url : 'http://localhost:8080/Tickets/hall/plan',
		type : 'post',
		dataType : 'json',
		success : function(data) {
			...
			for(var i=0;i<result.length;i++){
				$('#planBody').append('<tr class="unread"><td class="cell-icon hidden-phone hidden-tablet"></td>'+
						...
						'<button name="view" type="button" class="btn btn-primary" onclick="viewPlan('+i+')">查看</button>'+
						'</td><tr/>');
			}
...
function viewPlan(i){
	top.location="http://localhost:8080/Tickets/page/view_plan.html?planNo="+$('#planNo'+i).html();
}
...
function search(){
	...
	$.ajax({
		url : 'http://localhost:8080/Tickets/hall/searchPlan',
		type : 'post',
		dataType : 'json',
		data:{
			planNo:search
