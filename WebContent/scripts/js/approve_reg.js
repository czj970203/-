...
function init() {
	$.ajax({
		url : 'http://localhost:8080/Tickets/registry/regs',
		type : 'post',
		dataType : 'json',
		success : function(data) {
			...
			for(var i=0;i<result.length;i++){
				$('#regBody').append('<tr class="unread">'+
...
function approve(i){
	...
	$.ajax({
		url : 'http://localhost:8080/Tickets/manager/approve_reg',
		type : 'post',
		dataType : 'json',
		data:{
			hallNo:hallNo
...
function deny(i){
	var hallNo = $('#hallNo'+i).html();
	$.ajax({
		url : 'http://localhost:8080/Tickets/manager/deny_reg',
		type : 'post',
		dataType : 'json',
		data:{
			hallNo:hallNo
